...
resource "aws_instance" "clique_bootnode" {
  ami = "${data.aws_ami.ubuntu.id}"
  ...
  provisioner "file" {
    source = "files/besu_clique/besu.yml"
    destination = "$HOME/besu/besu.yml"
  }

  provisioner "file" {
    source = "files/besu_clique/clique.json"
    destination = "$HOME/besu/clique.json"
  }

  provisioner "file" {
    source = "files/besu_clique/bootnode"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sh $HOME/append_auth_keys.sh ${join(" ", formatlist("'%s'", var.user_ssh_public_keys))}",
      "sudo apt-get update && sudo apt-get install -y apparmor apt-transport-https ca-certificates curl build-essential python3 python3-setuptools python3-pip python3-dev python3-virtualenv python3-venv virtualenv",
      "wget https://cdn.azul.com/zulu/bin/zulu11.39.15-ca-jdk11.0.7-linux_amd64.deb && sudo dpkg -i zulu11.39.15-ca-jdk11.0.7-linux_amd64.deb"
      "sudo sh $HOME/besu/setup.sh '${var.besu_version}' '${var.besu_download_url}' '${aws_instance.clique_bootnode.private_ip}'",
      "sleep 30",
    ]
  }
}
...
resource "aws_route53_record" "bootnode_dns" {
  ...
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.clique_bootnode.private_ip}"]
}

resource "aws_instance" "clique_rpcnode" {
  ami = "${data.aws_ami.ubuntu.id}"
  instance_type = "${var.node_instance_type}"
  key_name = "${var.default_ssh_key}"
  ...
  provisioner "file" {
    source = "files/besu_clique/rpcnode"
    destination = "$HOME/besu/node_db/"
  }
...
resource "aws_route53_record" "rpcnode_dns" {
  ...
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.clique_rpcnode.private_ip}"]
}

resource "aws_instance" "clique_nodes" {
  ami = "${data.aws_ami.ubuntu.id}"
  instance_type = "${var.node_instance_type}"
  key_name = "${var.default_ssh_key}"
  ...
  provisioner "file" {
    source = "files/besu_clique/node-${count.index}"
    destination = "$HOME/besu/node_db/"
  }
...
resource "aws_route53_record" "nodes_dns" {
  ...
  type    = "A"
  ttl     = "300"
  records = ["${aws_instance.clique_nodes[count.index].private_ip}"]
}
